                ...
                @else
                    ...
                    <form action="{{ route('booking-room') }}" method="post" class="px-2">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" value="{{ \Carbon\Carbon::parse(session('check_in_date'))->diffInDays(\Carbon\Carbon::parse(session('check_out_date'))) }}" class="form-control my-2"
                                readonly>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="total_amount"
                                value="{{ ceil(\Carbon\Carbon::parse(session('check_in_date'))->diffInDays(\Carbon\Carbon::parse(session('check_out_date'))) * $roomDetail->price) }}"
                                class="form-control my-2" readonly>
                        </div>
                        @if (session('email'))
                            <div class="form-group">
                                <input type="submit" value="Đặt phòng" class="form-control my-3 submit"
                                    onclick="return confirm('Sau khi đặt phòng 30 phút nếu quý khách không thanh toán đơn sẽ tự động hủy!');">
                            </div>
                        @else
                ...
                <tbody>
                    @foreach ($listRoomHotelExceptId as $keyRoom => $valueRoom)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('room-detail', ['id' => $valueRoom->id]) }}" style="background-color:#86B817;padding:3px 5px;border-radius:3px;color:white;text-decoration:none;">Xem
                                    phòng</a>
                            </td>
                        </tr>
                    @endforeach
